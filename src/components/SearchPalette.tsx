...
import { ACC_DATA, TEAM } from "../data/accessories";
...
function pyFull(s: string): string {
  if (!s) return "";
  const cached = _pyCache.get("f:" + s);
...
function pyInitial(s: string): string {
  if (!s) return "";
  const cached = _pyCache.get("i:" + s);
  if (cached !== undefined) return cached;
...
*/
function fuzzyScore(text: string, needle: string) {
...
function entryScore(e: SearchEntry, q: string) {
  const scores = [
...
export default function SearchPalette({ open, onClose, wikiData, navigate }: { open: boolean; onClose: () => void; wikiData: WikiData; navigate: Navigate }) {
  ...
  const index = useMemo<SearchEntry[]>(() => {
    ...
    wikiData.items.forEach((i) =>
      idx.push({
        id: `item:${i.id}`,
        kind: "道具",
        name: i.name,
        sub: typeLabel(i.category),
        extra: `${(i.lore || []).join(" ")} ${JSON.stringify(i.stats || {})}`,
        page: "items",
        focus: i.id,
        icon: i.image?.path,
      }),
    ...
    wikiData.magics.forEach((m) =>
      (m.variants || []).filter((variant) => variant.id !== "unlock").forEach((v) =>
        idx.push({
          id: `magic:${m.id}:${v.id}`,
          kind: "魔法",
          name: `${m.name} · ${v.name}`,
    ...
    wikiData.talents.forEach((talent) => idx.push({
      id: `talent:${talent.id}`, kind: "天赋", name: talent.name,
      sub: `TALENT · ${String(talent.id).padStart(2, "0")}`,
      extra: `${talent.effect} ${talent.description}`, page: "talents", focus: String(talent.id), color: talent.color,
    }));
    wikiData.recipes.forEach((recipe) => idx.push({
      id: `recipe:${recipe.id}`, kind: "配方", name: recipe.result?.name || recipe.id,
      sub: recipe.type?.split(":").pop() || "RECIPE", extra: JSON.stringify(recipe.ingredients || []), page: "recipes", focus: recipe.id,
    }));
    wikiData.damage.forEach((record) => idx.push({
      id: `damage:${record.id}`, kind: "伤害", name: record.name, sub: record.label,
      extra: record.messages.join(" "), page: "systems", focus: record.id, color: record.color,
    }));
    // 规则页面 + 三阵营
    ...
    idx.push({ id: "page:recipes",  kind: "页面", name: "合成配方",   sub: "CRAFTING", page: "recipes" });
    idx.push({ id: "page:talents",  kind: "页面", name: "天赋档案",   sub: "TALENTS",  page: "talents" });
    idx.push({ id: "page:systems",  kind: "页面", name: "伤害机制",   sub: "DAMAGE",   page: "systems" });
  ...
  const showing: SearchEntry[] = q.trim()
    ? results
    : recent.map((r) => index.find((x) => x.id === r.id)).filter((entry): entry is SearchEntry => Boolean(entry));

  const pick = useCallback(
    (r: SearchEntry | undefined) => {
      if (!r) return;
      ...
      <div className="palette" role="dialog" aria-modal="true" aria-label="搜索全部档案" onClick={(e) => e.stopPropagation()}>
        <div className="palette-head">
          ...
          <input
            ...
            placeholder="搜索 · 道具 / 配方 / 魔法 / 天赋 / 任务 / 伤害 ……"
            value={q}
          ...
          {!q.trim() && recent.length === 0 && (
            <div className="palette-hint">
              ...
              <div className="palette-hint-sub">
                · 道具 {wikiData.items.length} 件
                · 饰品 {ACC_DATA.length} 件
                · 魔法变体 {wikiData.magics.reduce((count, magic) => count + magic.variants.length, 0)}
                · 天赋 {wikiData.talents.length}
                · 页面 11
              </div>
...
function GroupedResults({ list, active, q, onHover, pick }: { list: SearchEntry[]; active: number; q: string; onHover: (index: number) => void; pick: (entry: SearchEntry) => void }) {
...
function PaletteItem({ r, active, q, onHover, onClick }: { r: SearchEntry; active: boolean; q: string; onHover: () => void; onClick: () => void }) {
  return (
    <div
