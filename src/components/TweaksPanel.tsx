...
export default function TweaksPanel({ tweaks, update }: { tweaks: Tweaks; update: (edits: Partial<Tweaks>) => void }) {
  return (
    <div className="tweaks-panel open">
      ...
      <div className="variant-picker">
        {([
          ...
          {id: "paper",   label: "卷纸手抄"},
        ] as Array<{ id: Tweaks["variant"]; label: string }>).map((v) => (
          <div key={v.id}
      ...
      <div className="variant-picker" style={{gridTemplateColumns: "1fr 1fr"}}>
        {([
          ...
          {id: "compact",     label: "紧凑"},
        ] as Array<{ id: Tweaks["density"]; label: string }>).map((v) => (
          <div key={v.id}
