...
const SERVERS: Array<{ name: string; addr: string; note?: string }> = [
  { name: "默认地址", addr: "manosaba.tlspark.cn" },
];
    ...
    <section className="community-board">
      ...
      <div className="cb-grid-2">
        ...
        <div className="cb-card cb-server-card">
          ...
          <div className="cb-sub mono">内含多个地图游戏房间</div>
            ...
            {SERVERS.map((s) => (
              <div key={s.addr} className="cb-server-row">
                <div className="cb-sr-left">
                  ...
                  {s.note && <div className="cb-sr-note mono">{s.note}</div>}
                </div>
