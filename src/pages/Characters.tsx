...
import { ACC_DATA, COMBO_DATA, TEAM } from "../data/accessories";
import type { AccessoryRecord, ComboRecord } from "../data/accessories";
import type { StyleWithVariables } from "../types/wiki";

export default function CharactersPage({ focus }: { focus: string | null }) {
...
function AccessoryDetail({ a, relatedCombos }: { a: AccessoryRecord; relatedCombos: ComboRecord[] }) {
  return (
    <div className="char-card card" style={{ "--team-color": TEAM[a.team] } as StyleWithVariables}>
...
function CombosTable({ selected, setSelected }: { selected: AccessoryRecord; setSelected: Dispatch<SetStateAction<AccessoryRecord>> }) {
  const tiers: Array<{ id: ComboRecord["tier"]; label: string }> = [
    { id: "main",   label: "主联动"   },
    { id: "minor",  label: "二件套"   },
