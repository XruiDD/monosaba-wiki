...
import type { TaskRecord } from "../types/wiki";
...
function taskSortKey(t: TaskRecord): Array<[number, string]> {
  const cat = String(t.category || t.name || "");
  return cat.split("/").map((seg): [number, string] => {
...
function compareTaskKeys(a: TaskRecord, b: TaskRecord) {
...
export default function TasksPage({ tasksRaw, focus }: { tasksRaw: TaskRecord[]; focus: string | null }) {
